...
FVoxelDetailTextureManager* GVoxelDetailTextureManager = nullptr;

VOXEL_RUN_ON_STARTUP_GAME(RegisterVoxelDetailTextureManager)
{
	GVoxelDetailTextureManager = new FVoxelDetailTextureManager();
}
...
void UVoxelDetailTexture::PostDuplicate(const EDuplicateMode::Type DuplicateMode)
{
...
void FVoxelFloatDetailTextureRefPinType::Convert(
	...
	UVoxelFloatDetailTexture*& Object,
	FVoxelFloatDetailTextureRef& Struct) const
{
	if (bSetObject)
	{
		...
		{
			Object = Cast<UVoxelFloatDetailTexture>(Pool->WeakDetailTexture.Get());
		}
	}
	else
	{
...
void FVoxelColorDetailTextureRefPinType::Convert(
	const bool bSetObject,
	UVoxelColorDetailTexture*& Object,
	FVoxelColorDetailTextureRef& Struct) const
{
	if (bSetObject)
	{
		if (const TSharedPtr<FVoxelDetailTexturePool> Pool = Struct.WeakPool.Pin())
		{
			Object = Cast<UVoxelColorDetailTexture>(Pool->WeakDetailTexture.Get());
		}
	}
	else
	{
...
void FVoxelMaterialIdDetailTextureRefPinType::Convert(
	const bool bSetObject,
	UVoxelMaterialIdDetailTexture*& Object,
	FVoxelMaterialIdDetailTextureRef& Struct) const
{
	if (bSetObject)
	{
		if (const TSharedPtr<FVoxelDetailTexturePool> Pool = Struct.WeakPool.Pin())
		{
			Object = Cast<UVoxelMaterialIdDetailTexture>(Pool->WeakDetailTexture.Get());
		}
	}
	else
	{
...
void FVoxelNormalDetailTextureRefPinType::Convert(
	const bool bSetObject,
	UVoxelNormalDetailTexture*& Object,
	FVoxelNormalDetailTextureRef& Struct) const
{
	if (bSetObject)
	{
		if (const TSharedPtr<FVoxelDetailTexturePool> Pool = Struct.WeakPool.Pin())
		{
			Object = Cast<UVoxelNormalDetailTexture>(Pool->WeakDetailTexture.Get());
		}
	}
	else
	{
...
void FVoxelDetailTextureAllocator::Update_GameThread()
{
	...
	{
		Textures_GameThread[Index] = FVoxelTextureUtilities::CreateTexture2D(
	...
	DetailTextureMemory = Size * Size * GPixelFormats[PixelFormat].BlockBytes * NumTextures;
	...
	for (int32 Index = 0; Index < NumTextures; Index++)
	{
		...
		ENQUEUE_RENDER_COMMAND(FVoxelDetailTextureAllocator_Reallocate)([OldResource, NewResource, CopyInfo](FRHICommandListImmediate& RHICmdList)
		{
	...
	int32& OutPitch)
{
...

	{
		...
		OutData = MakeVoxelArrayView(UploadData).Slice(Index, UploadData.Num() - Index);
	}
...
void FVoxelDetailTextureUpload::BeginUpload_GameThread()
{
	...
	ENQUEUE_RENDER_COMMAND(FVoxelDetailTextureUpload_Upload)([This = AsShared(), Resource](FRHICommandListImmediate& RHICmdList)
	{
...
void FVoxelDetailTextureUpload::Upload_RenderThread(const FTextureResource& Resource)
{
	...
	for (const FVoxelDetailTextureAllocationRange& Range : Allocation->Ranges)
	{
		...
		RHIUpdateTexture2D(
			...
			UploadData.GetData() + TextureSize * Index * BytesPerPixel);
...
FVoxelDetailTexturePool::FVoxelDetailTexturePool(UVoxelDetailTexture& DetailTexture)
	...
	, Guid(*DetailTexture.Guid.ToString())
...
FString FVoxelDetailTextureManager::GetReferencerName() const
{
	return "FVoxelDetailTextureManager";
}
