...
float FVoxelUtilities::sRGBToLinearTable[256] =
{
	0.0f,
...
FFloatInterval FVoxelUtilities::GetMinMax(const TConstArrayView<float> Data)
{
	...
	ispc::BaseUtilities_GetMinMax(Data.GetData(), Data.Num(), &Result.Min, &Result.Max);
	return Result;
}
...
FFloatInterval FVoxelUtilities::GetMinMaxSafe(const TConstArrayView<float> Data)
{
	...
	FFloatInterval Result;
	ispc::BaseUtilities_GetMinMaxSafe(Data.GetData(), Data.Num(), &Result.Min, &Result.Max);
	return Result;
}