...
UEdGraphPin* UVoxelGraphNodeBase::GetInputPin(int32 Index) const
{
	return GetInputPins()[Index];
}

UEdGraphPin* UVoxelGraphNodeBase::GetOutputPin(int32 Index) const
{
...
void UVoxelGraphNodeBase::FocusOnNodes(const TArray<UEdGraphNode*>& Nodes)
{
	...
	const TSharedPtr<SGraphEditor> GraphEditor = Toolkit->FindGraphEditor(FirstNode->GetGraph());
