...
#include "Material/MaterialExpressionGetVoxelMaterial_TextureArray.h"
...
UMaterialExpressionGetVoxelMaterial_TextureArray::UMaterialExpressionGetVoxelMaterial_TextureArray()
{
	// Mitigate crash in UTexture::PostLoad
	static ConstructorHelpers::FObjectFinder<UTexture2DArray> DefaultTextureArray(
		TEXT("/Voxel/MaterialFunctions/DefaultTextureArray.DefaultTextureArray"));
...
UObject* UMaterialExpressionGetVoxelMaterial_TextureArray::GetReferencedTexture() const
{
	return FVoxelTextureUtilities::GetDefaultTexture2DArray();
}

void UMaterialExpressionGetVoxelMaterial_TextureArray::UpdateVoxelParameterData(
	...
	TVoxelInstancedStruct<FVoxelMaterialParameterData>& InOutParameterData) const
{
	...
	TVoxelArray<int32> IndirectionData;
	{
		...
		TVoxelArray<int32> FreeTextureIndices;
		{
			...
			ParameterData.TextureArrayValues.SetNumZeroed(UsedTextures.Num());
	...
	{
		ENQUEUE_RENDER_COMMAND(UpdateIndirectionTexture)([
			DebugName,
			...
			Resource = ParameterData.IndirectionTexture->GetResource()](FRHICommandListImmediate& RHICmdList)
		{
			...
			RHIUpdateTexture2D(
				...
				reinterpret_cast<const uint8*>(IndirectionData.GetData()));
		});
	}
	...
	for (int32 TextureIndex = 0; TextureIndex < ParameterData.TextureArrayValues.Num(); TextureIndex++)
	{
		...
		ENQUEUE_RENDER_COMMAND(UpdateTextureArray)([
			DebugName,
			NumMips,
			PixelFormat,
			TextureIndex,
...
#if WITH_EDITOR
void UMaterialExpressionGetVoxelMaterial_TextureArray::CompileVoxel(
	FMaterialCompiler& Compiler,
	UMaterialExpressionCustom& Custom,
	const FVoxelMaterialParameterData* ParameterData,
	const FName Name,
	const int32 MaterialIdValue,
	const int32 PreviewMaterialIdValue,
	TArray<int32>& Inputs)
{
	if (ParameterData &&
		!ensure(ParameterData->IsA<FVoxelMaterialParameterData_TextureArray>()))
	{
	...
	{
		EVoxelTextureArraySamplerType SamplerType;
		if (ParameterData)
		{
			...
			{
				SamplerType = EVoxelTextureArraySamplerType::Color;
			}
			break;
			case EVoxelTextureArrayCompression::BC5:
			{
				SamplerType = EVoxelTextureArraySamplerType::Normal;
			}
			break;
			}
		}
		...
		{
		default: ensure(false);
		case EVoxelTextureArraySamplerType::Color: return "ProcessMaterialColorTextureLookup";
		case EVoxelTextureArraySamplerType::Normal: return "UnpackNormalMap";
		}
	};

	Custom.OutputType = CMOT_Float4;

	if (MaterialIdValue == -1)
	{
		Custom.Code = R"(
...
// No derivatives in raytraced shaders, need to use SampleLevel
	...
	return UNPACKSAMPLE(TextureArray.Sample(TextureArraySampler, float3(UVs, Index)));
		...
		Inputs.Add(MaterialIdValue);
		Custom.Inputs.Add({ "MaterialId" });
	}

	Custom.Code.ReplaceInline(TEXT("UNPACKSAMPLE"), *SampleFunction);

	Inputs.Add(PreviewMaterialIdValue);
	Custom.Inputs.Add({ "PreviewMaterialId" });

	Inputs.Add(Compiler.TextureParameter(Name + "_IndirectionTexture", FVoxelTextureUtilities::GetDefaultTexture2D(), SAMPLERTYPE_Color));
	Custom.Inputs.Add({ "IndirectionTexture" });

	Inputs.Add(Compiler.TextureParameter(Name + "_TextureArray", FVoxelTextureUtilities::GetDefaultTexture2DArray(), SAMPLERTYPE_Color));
	Custom.Inputs.Add({ "TextureArray" });

	int32 UVsValue = UVs.Compile(&Compiler);
	if (UVsValue == -1)
	{
		UVsValue = Compiler.ComponentMask(
			Compiler.Div(
