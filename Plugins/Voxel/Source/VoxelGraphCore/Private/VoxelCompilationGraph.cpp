...
DEFINE_VOXEL_MEMORY_STAT(STAT_VoxelCompileGraph);
...
#define CHECK(...) if (!ensureVoxelSlow(__VA_ARGS__)) { VOXEL_MESSAGE(Error, "{0}: Internal error: {1}", this, #__VA_ARGS__); return; }
	...
	{
		static TVoxelSet<FPin*> StaticLinkedToSet;
	...
	{
		CHECK(Direction == EPinDirection::Input || Type.CanBeCastedTo(Pin->Type));
		CHECK(Direction == EPinDirection::Output || Pin->Type.CanBeCastedTo(Type));
...
void FGraph::Check()
{
	...
	static TVoxelAddOnlySet<FVoxelGraphNodeRef> VisitedRefs;
	ON_SCOPE_EXIT
	{
		VisitedRefs.Reset();
	};
	...
	for (FNode* Node : Nodes)
	{
		if (VisitedRefs.Contains(Node->NodeRef))
		{
			...
			VOXEL_MESSAGE(Error, "Duplicate node ref {0}", Node->NodeRef);
			return;
		}
